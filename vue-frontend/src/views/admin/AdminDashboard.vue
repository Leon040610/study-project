<template>
  <div class="admin-dashboard">
    ...
    <div v-loading="loading" class="dashboard-body">
      ...
      <el-card class="chart-card">
        <template #header>
          <div class="chart-header">
            <span>14 日活跃趋势</span>
            <div class="legend">
              <span class="legend-item"><i style="background:#3b82f6"></i>用户</span>
              <span class="legend-item"><i style="background:#10b981"></i>帖子</span>
              <span class="legend-item"><i style="background:#f59e0b"></i>资源</span>
            </div>
          </div>
        </template>
        <svg :viewBox="`0 0 ${chartW} ${chartH}`" class="line-chart" preserveAspectRatio="none">
          <g v-for="g in gridLines" :key="'g' + g.y">
            <line :x1="padX" :y1="g.y" :x2="chartW - padX" :y2="g.y" stroke="#e5e7eb" stroke-dasharray="2 3" />
            <text :x="padX - 6" :y="g.y + 4" font-size="10" fill="#9ca3af" text-anchor="end">{{ g.label }}</text>
          </g>
          <g v-for="(d, i) in daily" :key="'x' + i">
            <text :x="xAt(i)" :y="chartH - padX + 16" font-size="10" fill="#9ca3af" text-anchor="middle">
              {{ d.date.slice(5) }}
            </text>
          </g>
      ...
      <div class="grid-2">
        <el-card>
          <template #header><span>资源分类占比</span></template>
          ...
          <div v-else class="pie-row">
            <svg :viewBox="`0 0 200 200`" class="pie-chart">
              ...
              <text x="100" y="96" text-anchor="middle" font-size="13" fill="#6b7280">总数</text>
              <text x="100" y="118" text-anchor="middle" font-size="22" font-weight="700" fill="#111827">
                {{ stats.resourceTotal }}
              </text>
            </svg>
            <ul class="pie-legend">
...
.pie-name { flex: 1; color: #374151; }
.pie-value { font-weight: 600; color: #111827; }
...
.act-label { flex: 1; color: #374151; }
.act-value { font-weight: 700; color: #111827; font-size: 16px; }
