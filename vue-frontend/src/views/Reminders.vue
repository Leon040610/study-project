...
function stringToTime(s: string): Date | null {
  if (!s) return null
  // 兼容 "HH:mm" 新格式 与历史 ISO 字符串（自动取本地时区 HH:mm）
  if (/^\d{1,2}:\d{2}$/.test(s)) {
...
function formatTriggerCondition(rule: ReminderRule): string {
  ...
  // 关键修复：兼容新格式 "HH:mm" 与历史 ISO 字符串
  const hhmm = extractHHmm(rule.triggerTime)
  ...
  if (rule.triggerType === 'recurring') {
    ...
    return `${freqMap[rule.frequency || 'daily']} ${hhmm}`
  }
  return '-'
}

function extractHHmm(t: string | undefined | null): string {
  if (!t) return '00:00'
...
async function sendTestNotification() {
  ...
  try {
    // 关键修复：把前端当前选中的渠道传给后端，避免永远走默认偏好
    const res = await api.post('/reminders/test', { channel: prefs.defaultChannel }) as any
    ...
    // 推送状态反馈：详细展示每个渠道的发送结果
    if (res.logs && res.logs.length > 0) {
      const detail = res.logs.map((l: any) => {
        const ch = l.channel === 'email' ? '邮件' : l.channel === 'push' ? '手机推送' : l.channel
        return `${ch}: ${l.status === 'sent' ? '成功' : '失败'}${l.details ? ' (' + l.details + ')' : ''}`
      }).join('\n')
      console.log('[测试通知] 渠道结果:\n' + detail)
      // 失败渠道给出更醒目的提示
      const failed = res.logs.filter((l: any) => l.status !== 'sent')
      if (failed.length > 0) {
        ElMessageBox.alert(
          '成功: ' + (res.logs.length - failed.length) + ' / 失败: ' + failed.length + '\n' +
          detail,
          '推送状态详情',
...
async function saveRule() {
  // 关键修复：triggerTime 存为 "HH:mm" 字符串（时分循环概念），不再绑死到某个具体日期的 UTC 时间戳
  let triggerHHmm: string | undefined = undefined
  if (ruleForm.triggerType !== 'before_due' && ruleFormTime.value) {
    triggerHHmm = timeToString(ruleFormTime.value)
  }
